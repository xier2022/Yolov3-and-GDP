...
# 这段代码定义了一个名为YOLOv3的模型，它是一个用于目标检测的卷积神经网络。
# YOLOv3模型主要由两个部分组成：darknet53网络和三个残差块（resblock_body）。
# darknet53网络是一个深度可分离卷积网络，用于提取图像的特征。残差块用于增加网络的深度和表达能力。
#
# 在这段代码中，首先导入了一些必要的库，然后定义了一些辅助函数，
# 如DarknetConv2D、leakyRelu等。接下来，定义了YOLOv3模型的主体部分，
# 包括darknet_body、resblock_body、make_last_layers和yolo_body函数。
# 最后，通过调用yolo_body函数创建了一个YOLOv3模型实例。
