...
# DarknetConv2D是一个包装器，用于设置Darknet参数的卷积层。
# leakyRelu是一个激活函数，用于解决ReLU激活函数在负数区域梯度消失的问题。
# DarknetConv2D_BN_Leaky是一个组合了卷积、批量归一化和LeakyReLU的函数。
# resblock_body是一个残差块，包含两个卷积层和一个跳跃连接。
# darknet_body是一个包含多个残差块的函数，用于构建YOLOv3的主干网络。
# make_last_layers是一个函数，用于构建YOLOv3的最后一个阶段，包括6个卷积层和一个线性层。
# yolo_body是一个函数，用于构建YOLOv3的整个模型的主体部分。
# tiny_yolo_body是一个函数，用于构建YOLOv3的小型版本。
# yolo_head是一个函数，用于将最终层的输出转换为边界框参数。
# yolo_correct_boxes是一个函数，用于修正预测的边界框。
# yolo_boxes_and_scores是一个函数，用于计算YOLOv3模型的边界框和得分。
@wraps(Conv2D)
