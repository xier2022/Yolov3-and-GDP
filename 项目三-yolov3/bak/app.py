...
def detect_face():
    ...
    return {"code":200, "data":img_path.split("/")[-1], "msg":"登录成功。您好， " + username, "username":username}
...
def register():
    ...
    if result:
        ...
        msg = "注册成功！正在为您跳转。"
    yolo.save_single_face_feature(username, face_feature)
    img_path = get_path(username)
    if os.path.exists(img_path):
        os.remove(img_path)
    r_image = np.array(r_image)
...
def home_page(username, path):
    ...
    return render_template("home_page.html", username=username, img_path=path)
